...
class FirebaseImage extends ImageProvider<FirebaseImage> {
  ...
  static Reference _getImageRef(String location, FirebaseApp? firebaseApp) {
    FirebaseStorage storage = FirebaseStorage.instanceFor(app: firebaseApp, bucket: _getBucket(location));
  ...
  Future<Uint8List> _fetchImage() async {
    ...
    if (shouldCache) {
      ...
      FirebaseImageObject? localObject = await cacheManager.get(_imageObject.uri, this);

      if (localObject != null) {
        bytes = await cacheManager.localFileBytes(localObject);
        bytes ??= await cacheManager.upsertRemoteFileToCache(_imageObject, maxSizeBytes);
      } else {
        bytes = await cacheManager.upsertRemoteFileToCache(_imageObject, maxSizeBytes);
      }
    } else {
  ...
  bool operator ==(dynamic other) {
    ...
    return _imageObject.uri == typedOther._imageObject.uri && scale == typedOther.scale;
  }
